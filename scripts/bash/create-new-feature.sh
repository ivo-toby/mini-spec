...
BRANCH_NAME="${DATE_PREFIX}-${BRANCH_SUFFIX}"
...
if [ ${#BRANCH_NAME} -gt $MAX_BRANCH_LENGTH ]; then
    ...
    MAX_SUFFIX_LENGTH=$((MAX_BRANCH_LENGTH - 14))
    ...
    BRANCH_NAME="${DATE_PREFIX}-${TRUNCATED_SUFFIX}"
...
if $JSON_MODE; then
    printf '{"BRANCH_NAME":"%s","DESIGN_FILE":"%s","DATE_PREFIX":"%s"}\n' "$BRANCH_NAME" "$DESIGN_FILE" "$DATE_PREFIX"
else
